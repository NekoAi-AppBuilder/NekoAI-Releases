// supabase/functions/license-reset-device/index.ts
// Edge Function de Transferência/Reset de Dispositivo (v1.3.4 PROD)
// Invoca atomicamente fn_reset_license_device sem dependência de JWT de usuário
...
Deno.serve(async (req: Request) => {
  ...
  if (!supabaseUrl || !supabaseServiceKey) {
    console.error("[license-reset-device] Missing env: SUPABASE_URL present?", !!supabaseUrl, "SUPABASE_SERVICE_ROLE_KEY present?", !!supabaseServiceKey);
  ...
  // 1. Rate Limiting persistente por IP (10 tentativas/minuto)
  const rateLimitKey = `ip:${rateLimitIdentifier}:reset`;
  ...
  if (rlError) {
    console.error("[license-reset-device] RPC fn_check_rate_limit error:", {
  ...
  // 4. Invoca a função atômica PostgreSQL SECURITY DEFINER fn_reset_license_device
  const { data: dbResult, error: dbError } = await supabaseAdmin.rpc("fn_reset_license_device", {
  ...
  if (dbError) {
    console.error("[license-reset-device] RPC fn_reset_license_device error:", {
  ...
  if (!dbResult || !dbResult.ok) {
    ...
    return createJsonResponse({
      ...
      message: dbResult?.message || "Não foi possível transferir a licença.",
    }, statusCode);
  }

  // 5. Constrói o Grant Payload com grace_until estritamente <= expires_at
  const now = new Date();
  ...
  try {
    ...
    console.error("[license-reset-device] signGrant exception:", errMsg);
